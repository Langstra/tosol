...
class CsrfToken {
    ...
    public function generateToken() {
        ...
        if(!isset($_SESSION['csrf']) || !isset($_SESSION['csrf']['time']) || $_SESSION['csrf']['time'] < time() - $this->timeout ) {
            $_SESSION['csrf'] = array();
            $_SESSION['csrf']['time'] = time();
            $_SESSION['csrf']['salt'] = $this->randomString(32);
            $_SESSION['csrf']['sessid'] = session_id();
            $_SESSION['csrf']['ip'] = $_SERVER['REMOTE_ADDR'];
            $_SESSION['csrf_hash'] = $this->calculateHash();
        }
        // Generate and return the token
        return base64_encode($_SESSION['csrf_hash']);
    }
        ...
        if (isset($_SESSION['csrf'])) {
            ...
            if (session_id()) {
                ...
                if (($this->acceptGet and $isCsrfGet) or $isCsrfPost) {
                    ...
                    $generatedHash = $_SESSION['csrf_hash'];
