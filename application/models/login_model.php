...
class LoginModel
{
    ...
    public function login($username, $password, $remember)
    {
        ...
        if (!isset($username) OR empty($username)) {
            $_SESSION[SES_FEEDBACK_NEGATIVE][] = FEEDBACK_USERNAME_FIELD_EMPTY;
            return false;
        }
        if (!isset($password) OR empty($password)) {
            $_SESSION[SES_FEEDBACK_NEGATIVE][] = FEEDBACK_PASSWORD_FIELD_EMPTY;
            return false;
        }
        ...
        if ($count != 1) {
            ...
            $_SESSION[SES_FEEDBACK_NEGATIVE][] = FEEDBACK_LOGIN_FAILED;
            return false;
        }
        ...
        if (($result->failed_logins >= 3) AND ($result->last_failed_login > $result->CURRENT_TIMESTAMP)) {
            $_SESSION[SES_FEEDBACK_NEGATIVE][] = FEEDBACK_PASSWORD_WRONG_3_TIMES;
            return false;
        }
        ...
        if (password_verify($password, $result->password)) {
            ...
            if ($result->subscription_date === NULL) {
                $_SESSION[SES_FEEDBACK_NEGATIVE][] = FEEDBACK_ACCOUNT_NOT_ACTIVATED_YET;
                return false;
            }

            // login process, write the user data into session
            Session::init();
            Session::set(SES_LOGGED_IN, true);
            Session::set(SES_USER_ID, $result->user_id);
            Session::set(SES_USER_NAME, $result->nickname);
            Session::set(SES_ACTIVATED, $result->subscription_date);

            // reset the failed login counter for that user (if necessary)
            if ($result->last_failed_login > 0) {
                $sql = "UPDATE user SET failed_logins = 0, last_failed_login = NULL
                        WHERE user_id = :user_id AND failed_logins != 0";
            ...
            $sql = "UPDATE user SET last_login = CURRENT_TIMESTAMP WHERE user_id = :user_id";
            $sth = $this->db->prepare($sql);
            $sth->execute(array(':user_id' => $result->user_id));
            ...
            if (isset($remember) && $remember) {
                ...
                $sql = "UPDATE user SET rememberme_token = :user_rememberme_token WHERE user_id = :user_id";
                $sth = $this->db->prepare($sql);
            ...
            $sql = "UPDATE user
                    SET failed_logins = failed_logins+1, last_failed_login = CURRENT_TIMESTAMP
                    WHERE nickname = :user_name";
            $sth = $this->db->prepare($sql);
            $sth->execute(array(':user_name' => $username));
            // feedback message
    ...
    public function editUserName($username)
    {
        ...
        if (!preg_match("/^(?=.{2,64}$)[a-zA-Z][a-zA-Z0-9]*(?: [a-zA-Z0-9]+)*$/", $username)) {
            $_SESSION["feedback_negative"][] = FEEDBACK_USERNAME_DOES_NOT_FIT_PATTERN;
            return false;
        }

        // clean the input
        $user_name = substr(strip_tags($username), 0, 64);
